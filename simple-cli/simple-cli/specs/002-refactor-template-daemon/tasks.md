...
- [ ] T020 [US1] [US2] Edit `cmd/root.go` — remove the two session import lines:
      `sessioncmd "github.com/binpqh/simple-cli/cmd/session"` and
      `"github.com/binpqh/simple-cli/internal/session"`. Remove the package-level
...
- [ ] T021 [US1] [US2] _(depends on T020)_ Edit `cmd/root.go` — update `rootCmd` metadata:
  - `Use`: keep `"simple-cli"`
...
- [X] T025 [US1] Delete file `tests/integration/session_test.go` entirely (contains
      `TestSessionLifecycle`, `TestSessionNotFound`, and the `TestMain`+`run` helpers that
      reference `SIMPLE_CLI_STATE_DIR`).

- [X] T026 [US1] Delete file `tests/integration/quickstart_validation_test.go` entirely
      (contains `TestQuickstartValidation` which exercises `session start`, `session list`,
      `session resume`, `session stop`, `session reset`).

- [X] T027 [US1] [US3] Rewrite `tests/integration/output_test.go` — keep the `//go:build integration`
...
- [X] T028 [US1] [US3] Create new file `tests/integration/run_test.go` — `//go:build integration`,
...
- [X] T029 [P] [US2] Edit `cmd/root.go` — add an inline doc comment block above the
...
- [X] T030 [P] [US2] Edit `cmd/run.go` — expand the TODO comment to a multi-line guide:
...
- [X] T044 [P] [US2] Create `cmd/example_cmd.go` — a minimal, fully documented example
...
- [X] T031 [P] [US3] Edit `README.md` — replace the feature overview, quick-start commands,
...
- [X] T032 [P] [US3] Edit `docs/quickstart.md` — replace the 5-step session walkthrough
...
- [X] T033 [P] [US3] Edit `docs/configuration.md` — remove the `state_dir` row from the
...
- [X] T034 [P] [US3] Edit `docs/architecture.md` — remove the session store architecture
...
- [X] T035 [P] [US3] Edit `docs/ai-agent-guide.md` — remove all `session` command examples.
...
- [X] T036 [P] [US3] Edit `CHANGELOG.md` — add an `## [Unreleased]` section (or update
...
- [X] T037 [P] Audit `Makefile` — check all targets for references to deleted packages
...
- [X] T038 [P] Delete artefact files in repo root that are session-specific remnants:
...
- [X] T039 Run `go build ./...` and fix any remaining compile errors. Expected: zero errors.
...
- [X] T040 Run `go test ./internal/... ./pkg/... -coverprofile=coverage.out -covermode=atomic`
      and verify: zero test failures, coverage ≥ 80% on `internal/` and `pkg/` packages.

- [X] T041 Run `go test -tags integration ./tests/integration/...` (requires built binary at
...
- [X] T042 Run `golangci-lint run ./...` and fix any reported issues. Expected: zero issues.
      Common: unused import after session removal, missing GoDoc on `newRunCmd`.

- [X] T043 **Version bump — Principle X compliance**: Update `CHANGELOG.md` to replace
